...
;// CONCATENATED MODULE: ./src/js/components/tableSlider-hero.js
...
typeButtons.forEach(btn => {
  btn.addEventListener("click", () => {
    ...
    if (isHeroOnPage) {
      lib_axios.get(`http://localhost:3000/methchivDataNew?type=${activeTypeButton.innerHTML}`).then(r => {
        sliderData = r.data;
        makeSlider({
          updateSlides: true
        });
...
`;
if (isHeroOnPage) {
  lib_axios.get(`http://localhost:3000/methchivData?type=${activeTypeButton.innerHTML}`).then(r => {
    sliderData = r.data;
    makeSlider();
  }).catch(e => {
    console.log(e);
